...
class RadioRange {
    ...
    double RadiusProbability(vector <int> X, vector <int> Y, vector <int> R, int Z) {
    	...
    	for(int i=0;i<X.size();i++) {
    		...
    		if(num>num1)
    			v.push_back(ii(num-num1,num+num1));
    		else
				v.push_back(ii(0,num+num1));    			
			// cout<<v[i].first<<" "<<v[i].second<<endl;
    	}

    	double ans=0,num=Z,big=0;
    	...
    	for(int i=0;i<v.size();i++) {
    		if(v[i].first>num)
    			break;
    		if(v[i].first>=big) {
	    		ans+=min(v[i].second,num)-v[i].first;
	    		big=min(v[i].second,num);
    		}
    		else if(big>=v[i].second) {
    			continue;
    		}
    		else {
    			ans+=min(v[i].second,num)-big;
    			big=min(v[i].second,num);
    		}
    	}
    	ans/=num;
